...
export function ReproductionPage({ papers, notify, openPaper, initialPaperId }: ReproductionPageProps) {
  ...
  const [runForm, setRunForm] = useState({ environment: '', command: '', parameters: '', dataVersion: '', codeRevision: '', seed: '', status: 'completed' as ExperimentRun['status'], metrics: '', resultSummary: '' });
  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      ...
      setError('');
      if (initialPaperId && !initialPaperHandled.current) {
  ...
  }, [initialPaperId, query, statusFilter, tagFilter, sort]);
  ...
  const createProject = async () => {
    ...
    setCreateBusy(true);
    try {
      const project = await reproductionApi.create({ name, paperId: createPaperId || null, tags: parseTags(createTags) });
      setCreateOpen(false);
      setCreateName('');
      setCreatePaperId('');
      setCreateTags('');
  ...
  const recordRun = async () => {
    ...
    try {
      ...
      const run = await reproductionApi.createRun(selected.id, {
        ...runForm,
        parameters,
        metrics,
      ...
      setRunOpen(false);
      setRunForm({ environment: '', command: '', parameters: '', dataVersion: '', codeRevision: '', seed: '', status: 'completed', metrics: '', resultSummary: '' });
      notify('实验运行已记录');
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : String(reason));
    } finally {
  ...
  return (
    <div className="page reproduction-page">
      <header className="reproduction__header">
        <div>
          <span className="eyebrow">PAPER REPRODUCTION</span>
          <h1 className="display-title">论文复现</h1>
          <p>记录实验、结果与复现笔记，让每次尝试都可追溯。</p>
        </div>
        <button type="button" className="btn btn--primary" onClick={() => setCreateOpen(true)}><PlusIcon size={15} /> 新建复现</button>
      </header>
      ...
      <div className="reproduction__toolbar">
        ...
        <label className="reproduction__filter"><span className="sr-only">项目排序</span><select className="input" value={sort} onChange={(event) => setSort(event.target.value as 'updated' | 'name')}><option value="updated">最近更新</option><option value="name">项目名称</option></select></label>
        <label className="reproduction__filter"><span className="sr-only">项目标签</span><input className="input" value={tagFilter} onChange={(event) => setTagFilter(event.target.value)} placeholder="按标签筛选" /></label>
        <label className="reproduction__filter"><span className="sr-only">项目排序</span><select className="input" value={sort} onChange={(event) => setSort(event.target.value as 'updated' | 'name')}><option value="updated">最近更新</option><option value="name">项目名称</option></select></label>
        <span className="reproduction__count">{loading ? '正在加载…' : `${projects.length} 个项目`}</span>
      </div>

      <div className="reproduction__workspace">
        ...
        {selected && <>
          <section className="reproduction__editor" aria-label="复现文档编辑器" aria-busy={detailLoading}>
            ...
            <footer className="reproduction__editor-foot"><span>Markdown · KaTeX · 自动保存</span><span>{draft.length.toLocaleString()} 字</span></footer>
          </section>

          <aside className="reproduction__inspector" aria-label="复现项目大纲与摘要" aria-busy={detailLoading}>
            <section className="reproduction__inspector-section"><div className="reproduction__section-head"><div><span className="eyebrow">OUTLINE</span><h2>文档大纲</h2></div><span>{outline.length} 节</span></div>{outline.length === 0 ? <p className="reproduction__muted">在文档中添加 Markdown 标题后会显示章节导航。</p> : <nav aria-label="复现文档大纲"><ol className="reproduction__outline">{outline.map((item) => <li key={item.id} className={`reproduction__outline-level-${item.level}${activeHeadingId === item.id ? ' is-active' : ''}`}><button type="button" aria-current={activeHeadingId === item.id ? 'location' : undefined} onClick={() => jumpToHeading(item.label, item.id)}>{item.label}<ArrowRightIcon size={12} /></button></li>)}</ol></nav>}</section>
            <section className="reproduction__inspector-section reproduction__summary"><span className="eyebrow">PROJECT STATUS</span><div className="reproduction__summary-status"><StatusBadge status={selected.status} /><select className="input input--sm" aria-label="项目状态" value={selected.status} disabled={selected.status === 'archived'} onChange={(event) => void updateStatus(event.target.value as ReproductionStatus)}>{Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><p>最后更新 {formatDate(selected.updatedAt)} · 修订 {selected.revision}</p></section>
            <section className="reproduction__inspector-section"><div className="reproduction__section-head"><div><span className="eyebrow">EXPERIMENTS</span><h2>实验运行</h2></div><button type="button" className="btn btn--primary btn--sm" disabled={!canEdit} onClick={() => setRunOpen(true)}><SparkIcon size={13} /> 记录</button></div>{runs.length === 0 ? <p className="reproduction__muted">还没有实验运行记录。</p> : <ol className="reproduction__run-list">{runs.slice(0, runsExpanded ? runs.length : 4).map((run) => <li key={run.id}><span className={`reproduction__run-dot reproduction__run-dot--${run.status}`} aria-hidden="true" /><span><strong>{run.resultSummary || '实验运行'}</strong><small>{run.environment || '未记录环境'} · {formatDate(run.createdAt)}</small></span><b>{RUN_STATUS_LABELS[run.status]}</b></li>)}</ol>}{runs.length > 4 && <button type="button" className="btn btn--ghost btn--sm reproduction__full-link" aria-expanded={runsExpanded} onClick={() => setRunsExpanded((expanded) => !expanded)}>{runsExpanded ? '收起运行记录' : `查看全部 ${runs.length} 次运行`} <ArrowRightIcon size={12} /></button>}</section>
            <section className="reproduction__inspector-section"><div className="reproduction__section-head"><div><span className="eyebrow">ATTACHMENTS</span><h2>附件</h2></div><button type="button" className="btn btn--ghost btn--sm" disabled={!canEdit} onClick={() => setArtifactOpen(true)}><PlusIcon size={13} /> 添加</button></div>{artifacts.length === 0 ? <p className="reproduction__muted">尚未添加附件。</p> : <ul className="reproduction__artifact-list">{artifacts.slice(0, artifactsExpanded ? artifacts.length : 4).map((artifact) => <li key={artifact.id}><span><a href={reproductionApi.artifactUrl(artifact.projectId, artifact.id)} download={artifact.filename} aria-label={`下载附件 ${artifact.filename}`}><DownloadIcon size={12} /> {artifact.filename}</a><small>{Math.ceil(artifact.sizeBytes / 1024)} KB · {artifact.mimeType}</small></span><code>{artifact.kind}</code></li>)}</ul>}{artifacts.length > 4 && <button type="button" className="btn btn--ghost btn--sm reproduction__full-link" aria-expanded={artifactsExpanded} onClick={() => setArtifactsExpanded((expanded) => !expanded)}>{artifactsExpanded ? '收起附件' : `查看全部 ${artifacts.length} 个附件`} <ArrowRightIcon size={12} /></button>}</section>
            <section className="reproduction__inspector-section"><div className="reproduction__section-head"><div><span className="eyebrow">NOTES</span><h2>复现笔记</h2></div><button type="button" className="btn btn--ghost btn--sm" disabled={!canEdit} onClick={() => setNoteOpen(true)}>新增</button></div>{notes.length === 0 ? <p className="reproduction__muted">暂无独立笔记。</p> : <ul className="reproduction__note-list">{notes.slice(0, notesExpanded ? notes.length : 3).map((note) => <li key={note.id}><p>{note.content}</p><small>{formatDate(note.createdAt)}</small></li>)}</ul>}{notes.length > 3 && <button type="button" className="btn btn--ghost btn--sm reproduction__full-link" aria-expanded={notesExpanded} onClick={() => setNotesExpanded((expanded) => !expanded)}>{notesExpanded ? '收起笔记' : `查看全部 ${notes.length} 条笔记`} <ArrowRightIcon size={12} /></button>}</section>
      ...
      {createOpen && <div className="reproduction__dialog-backdrop" role="presentation" onClick={(event) => { if (!createBusy && event.target === event.currentTarget) setCreateOpen(false); }}><form ref={createDialogRef} className="reproduction__dialog" role="dialog" aria-modal="true" aria-labelledby="create-reproduction-title" onSubmit={(event) => { event.preventDefault(); void createProject(); }}><div className="reproduction__dialog-head"><div><span className="eyebrow">NEW PROJECT</span><h2 id="create-reproduction-title">新建论文复现</h2></div><button type="button" className="btn btn--ghost btn--sm" aria-label="关闭新建复现对话框" disabled={createBusy} onClick={() => setCreateOpen(false)}><CloseIcon size={15} /></button></div><label>项目名称<input className="input" autoFocus required value={createName} onChange={(event) => setCreateName(event.target.value)} placeholder="例如：ViT 分阶段训练复现" /></label><label>关联论文（可选）<select className="input" value={createPaperId} onChange={(event) => setCreatePaperId(event.target.value)}><option value="">暂不关联</option>{papers.map((item) => <option key={item.id} value={item.id}>{item.title_zh || item.title}</option>)}</select></label><label>标签（可选）<input className="input" value={createTags} onChange={(event) => setCreateTags(event.target.value)} placeholder="用逗号分隔，例如：视觉, 基线" /></label><p className="reproduction__dialog-hint">创建后会生成一份空白复现模板，不会伪造实验内容。</p><div className="reproduction__dialog-actions"><button type="button" className="btn btn--ghost" disabled={createBusy} onClick={() => setCreateOpen(false)}>取消</button><button type="submit" className="btn btn--primary" disabled={createBusy || !createName.trim()}>{createBusy ? '创建中…' : '创建项目'}</button></div></form></div>}
      ...
      {runOpen && <div className="reproduction__dialog-backdrop" role="presentation" onClick={(event) => { if (!runBusy && event.target === event.currentTarget) setRunOpen(false); }}><form ref={runDialogRef} className="reproduction__dialog reproduction__dialog--wide" role="dialog" aria-modal="true" aria-labelledby="record-run-title" onSubmit={(event) => { event.preventDefault(); void recordRun(); }}><div className="reproduction__dialog-head"><div><span className="eyebrow">EXPERIMENT RUN</span><h2 id="record-run-title">记录实验运行</h2></div><button type="button" className="btn btn--ghost btn--sm" aria-label="关闭记录运行对话框" disabled={runBusy} onClick={() => setRunOpen(false)}><CloseIcon size={15} /></button></div><div className="reproduction__form-grid">{([['environment', '运行环境'], ['command', '命令'], ['parameters', '参数'], ['dataVersion', '数据版本'], ['codeRevision', '代码提交'], ['seed', '随机种子']] as const).map(([key, label]) => <label key={key}>{label}<input className="input" autoFocus={key === 'environment'} value={runForm[key]} onChange={(event) => setRunForm((current) => ({ ...current, [key]: event.target.value }))} /></label>)}<label>状态<select className="input" value={runForm.status} onChange={(event) => setRunForm((current) => ({ ...current, status: event.target.value as ExperimentRun['status'] }))}>{Object.entries(RUN_STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>指标<textarea className="input" rows={2} value={runForm.metrics} onChange={(event) => setRunForm((current) => ({ ...current, metrics: event.target.value }))} /></label><label className="reproduction__form-wide">结果摘要<textarea className="input" rows={3} value={runForm.resultSummary} onChange={(event) => setRunForm((current) => ({ ...current, resultSummary: event.target.value }))} /></label></div><div className="reproduction__dialog-actions"><button type="button" className="btn btn--ghost" disabled={runBusy} onClick={() => setRunOpen(false)}>取消</button><button type="submit" className="btn btn--primary" disabled={runBusy}>{runBusy ? '保存中…' : '保存运行'}</button></div></form></div>}
