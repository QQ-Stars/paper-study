# Paper-Study(VibeCoding项目) · 文献管理 · 学术搜索 · 论文精读
...
| **导入本地 PDF** | 管理页「📂 本地 PDF 批量导入」→ 填写文件夹路径 → 扫描 → 勾选 → 导入 |
| **趋势与空白分析** | 「洞察」页查看趋势与引用图；或配置 [MCP](#七mcp-服务可选) 由 Claude 协助分析 |
...
- **数据持久化**：论文库 / PDF / 设置均存于项目的 `data/` 目录（挂载进容器），重启容器后保留。
