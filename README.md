...
- 📖 **精读**：左侧论文列表 · 中间内嵌 PDF（PDF.js）· 右侧「论文讲解 / 译文 / 我的笔记」三栏。
...
- **流式接口**走 NDJSON：检索 `/api/search`、入库 `/api/ingest-selected`、会议核实 `/api/verify-venue`、讲解 `/api/explain`、翻译 `/api/translate` 都把进度逐行推给前端做动画。
