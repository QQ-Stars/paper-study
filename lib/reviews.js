...
function createReviewStore(db) {
  ...
  const titleZh = titleZhColumn ? 'p.title_zh' : 'NULL';
  ...
  `);
  const understoodProgressWithoutPlan = db.prepare(`
    SELECT progress.paper_id, progress.updated_at
    FROM progress
    JOIN papers ON papers.id = progress.paper_id
    LEFT JOIN paper_reviews r ON r.paper_id = progress.paper_id
    WHERE progress.status = ?
      AND r.paper_id IS NULL
    ORDER BY progress.updated_at ASC, progress.paper_id ASC
  `);
  const backfillTx = db.transaction((rows, updated) => {
    let created = 0;
    for (const row of rows) {
      const start = dateOnly(row.updated_at || updated);
      const due = scheduleForStep(start, 1);
      created += insert.run(row.paper_id, start, due, updated).changes;
    }
    return created;
  });
  ...
  function ensureReviewPlan(paperId, { now = new Date(), startedAt = null } = {}) {
    const existing = getReviewPlan(paperId);
    ...
    return getReviewPlan(paperId);
  }

  function ensureReviewPlanForStatus(paperId, status, options = {}) {
    if (status !== STATUS_UNDERSTOOD) return null;
    return ensureReviewPlan(paperId, options);
  }

  function backfillUnderstoodReviews({ now = new Date() } = {}) {
    const updated = dateOnly(now);
    const rows = understoodProgressWithoutPlan.all(STATUS_UNDERSTOOD);
    return backfillTx(rows, updated);
  }
  ...
  return {
    backfillUnderstoodReviews,
    completeReviewStep,
    ensureReviewPlan,
    ensureReviewPlanForStatus,
    getReviewPlan,
    listReviewItems
  };
}
